...
_mcf_filters = [Chem.MolFromSmarts(x) for x in _mcf['smarts'].values]
_pains_filters = [Chem.MolFromSmarts(x) for x in _pains['smarts'].values]
...
def mcf_filter(mol):
    ...
    if any(mol.HasSubstructMatch(smarts) for smarts in _mcf_filters):
        return 0
    else:
        return 1
...
def pains_filter(mol):
    if type(mol) == str:
        mol = Chem.MolFromSmiles(mol)
    if any(mol.HasSubstructMatch(smarts) for smarts in _pains_filters):
        return 0
    else:
        return 1